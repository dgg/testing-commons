...
{
	[TestFixture]
	public class DataContractSerializationConstraintTester : Support.ConstraintTesterBase
	{
		#region ApplyTo
		...
		public void ApplyTo_MatchingSerializable_True()
		{
			...
			var subject = new SerializationConstraint<Serializable>(new DataContractRoundtripSerializer<Serializable>(),
		...
		public void ApplyTo_NonSerializable_False()
		{
			...
			var subject = new SerializationConstraint<NonSerializable>(new DataContractRoundtripSerializer<NonSerializable>(), Is.Not.Null);
		...
		public void CanBeNewedUp()
		{
			Assert.That(new Serializable { D = 3m, S = "s" },
				new SerializationConstraint<Serializable>(new DataContractRoundtripSerializer<Serializable>(),
		...
		{
			Assert.That(new Serializable { D = 3m, S = "s" },
				Must.Be.DataContractSerializable<Serializable>(
					Has.Property("S").EqualTo("s")
						.And.Property("D").EqualTo(3m)));
		}
	}
