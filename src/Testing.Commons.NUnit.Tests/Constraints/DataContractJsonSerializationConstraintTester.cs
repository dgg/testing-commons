...
{
	[TestFixture]
	public class DataContractJsonSerializationConstraintTester : Support.ConstraintTesterBase
	{
		#region ApplyTo
		...
		public void ApplyTo_MatchingSerializable_True()
		{
			...
			var subject = new SerializationConstraint<Serializable>(new DataContractJsonRoundtripSerializer<Serializable>(),
		...
		public void ApplyTo_NonSerializable_False()
		{
			...
			var subject = new SerializationConstraint<NonSerializable>(new DataContractJsonRoundtripSerializer<NonSerializable>(), Is.Not.Null);
		...
		public void CanBeNewedUp()
		{
			Assert.That(new Serializable { D = 3m, S = "s" },
				new SerializationConstraint<Serializable>(new DataContractJsonRoundtripSerializer<Serializable>(),
		...
		{
			Assert.That(new Serializable { D = 3m, S = "s" },
				Must.Be.DataContractJsonSerializable<Serializable>(
					Has.Property("S").EqualTo("s")
						.And.Property("D").EqualTo(3m)));
		}
	}
