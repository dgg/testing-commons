...
{
	[TestFixture]
	public class MatchingConstraintTester_ProblemExploration
	{
		[Test]
		...
		private Constraint addressWithZipCode(string zipCode)
		{
		...
		public void UsingCustomEqualityComparer_Works_ButFeedbackWhenFailingIsPoor()
		{
			...
			string typeName = string.Format("<{0}>", typeof(CustomerWithCollection).FullName);
			Assert.That(() => Assert.That(actual, Is.EqualTo(expected).Using(new CustomComparer())),
				Throws.InstanceOf<AssertionException>().With.Message.StringContaining(
