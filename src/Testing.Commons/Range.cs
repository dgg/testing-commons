...
internal class Range<T> where T : IComparable<T>
{
	...
	{
		if (!checkBounds(lowerBound, upperBound)) throw new ArgumentOutOfRangeException(nameof(upperBound), upperBound, Resources.Exceptions.UnorderedRangeBounds);
	}
	...
	{
		return string.Format(CultureInfo.InvariantCulture, "[{0}..{1}]", LowerBound, UpperBound);
	}
}
